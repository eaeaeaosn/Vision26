...
void capture_loop(const std::string & config_path, const std::string & output_folder)
{
  ...
  while (true) {
    ...
    // 不控制云台（control=false），仅作为心跳以保持串口链路，然后读取插值后的四元数
    ...
    auto success = cv::findChessboardCorners(img, pattern_size, corners);  // 棋盘格内角点
    cv::drawChessboardCorners(img_with_ypr, pattern_size, corners, success);  // 显示识别结果
